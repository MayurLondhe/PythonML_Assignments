...
def directroyCheckSum(DirectoryName):
    Border = "-"*50
    timestamp = time.ctime()

    LogFileName = "DirectoryChecksum_%s.log" %(timestamp)
    ...
    fobj.write("This is Directroy Checksum Automation\n")
    fobj.write(Border+"\n")
    ...
    for FolderName, SubFolderName, FileName in os.walk(DirectoryName):
        ...
        for fname in FileName:
            ...
            print(f"Check sum of {fname} is {checkSum}")
            fobj.write(f"Check sum of {fname} is {checkSum} \n")
...
def main():
    Border = "-"*40
    print(Border)
    print("----- Directroy Checksum Automation ----")
    print(Border)
    if(len(sys.argv) > 0):
        if((sys.argv[1] == "--h") or (sys.argv[1] == "--H")):
            ...
            directroyCheckSum(DirectoryName)
